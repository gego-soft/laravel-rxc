...
class Client
{
    ...
    public function __construct($config = [])
    {
        ...
        $handlerStack->push(
            Middleware::mapResponse(function (ResponseInterface $response) {
                return RxccoindResponse::createFrom($response);
            }),
            'json_response'
        );
    ...
    public function request($method, $params = [])
    {
        try {
           $uri='/';
            ...
            if ($response->hasError()) {
                ...
                throw new Exceptions\RxccoindException($response->error());
            }
            ...
            if (
                ...
                throw new Exceptions\RxccoindException($exception->getResponse()->error());
            }
        ...
        } catch (Exceptions\RxccoindException $exception) {
            throw $exception;
        }
    }

    /**
     * Makes async request to rxccoin Core.
     *
     * @param string        $method
     * @param mixed         $params
        ...
        callable $onRejected = null)
    {
        ...
        $promise->then(
            function (ResponseInterface $response) use ($onFullfiled) {
                $error = null;
                if ($response->hasError()) {
                    $error = new Exceptions\RxccoindException($response->error());
                }
            ...
            function (RequestException $exception) use ($onRejected) {
                if (
                    ...
                    $exception = new Exceptions\RxccoindException(
    ...
    /**
     * Converts amount from satoshi to rxccoin.
     *
     * @param int $amount
     *
     * @return float
    ...
    /**
     * Converts amount from rxccoin to satoshi.
     *
     * @param float $amount
     *
     * @return int
